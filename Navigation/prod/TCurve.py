...
# internal methods
    ...
    def integrate(self, t, n, f):
        ...
        lowbound = 0
        highbound = t
        while(abs(simNew-simOld)/simNew > epsilon):
            w = float(highbound -lowbound)/s
            simOld=simNew
            for i in range(1,s):
                if ( i == 1):
                    simNew = (w/3)*(self.f(lowbound,n)+self.f(highbound,n) + 4*self.f(lowbound+i*w,n))
                elif (i%2 == 1):
                    simNew += 4*w*self.f(lowbound+i*w,n)/3
                else:
                    simNew += 2*w*self.f(lowbound+i*w,n)/3
            s = s *2
        return simNew
